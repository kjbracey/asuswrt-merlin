...
int main(int argc,char **argv) {
    ...
    if(argc < 2) {
        ...
        fprintf(stderr,"This program listens for broadcast  packets  on the  specified UDP port\n"
            ...
            "            Setting to 240.0.0.1 uses outgoing interface address and broadcast port.\n"
            "            (helps in some rare cases)\n"
            "            Setting to 240.0.0.2 uses outgoing interface address and source port.\n"
            "            (helps in some rare cases)\n"
            "\n"
        );
        exit(1);
    ...
    for (;;) /* endless loop */
    {
        ...
        for (iIf = 0; iIf < maxifs; iIf++) {
            ...
            u_short fromPort;
            if (spoof_addr == inet_addr("240.0.0.1")) {
                fromAddress = iface->ifaddr;
                fromPort = port;
            } else if (spoof_addr == inet_addr("240.0.0.2")) {
                fromAddress = iface->ifaddr;
